...
void main() {
  ...
  testWidgets(
    'draws 3 donut segments and legend percents for folder 10 / used 60 / free 40',
    (tester) async {
  ...
  testWidgets(
    'places folder percent outside the ring when the slice is under 5%',
    (tester) async {
      await tester.pumpWidget(
        wrapChart(
          StorageChart(
  ...
  testWidgets(
    'shows folder skeleton and Used/Free legend while getSize is pending',
    (tester) async {
  ...
  testWidgets(
    'shows storageScanError and a 2-slice donut when scan fails without cache',
    (tester) async {
  ...
  testWidgets(
    'empty snapshot shows 0 files, hides top folders, omits folder pie slice',
    (tester) async {
      await tester.pumpWidget(
        wrapChart(
          StorageChart(
      ...
      expect(
        pie.any((s) => s.color == ThemePresets.midnight.warning && s.value > 0),
        isFalse,
      );
      expect(find.text('Folder'), findsOneWidget);
